...
inline int dlock_server::check_recv_cr_status(urma_cr_t *cr, int idx, bool ssl_enable)
{
    ...
    if (cr[idx].status != URMA_CR_SUCCESS) {
        DLOCK_LOG_ERR("cr failed status: 0x%x", static_cast<int>(cr[idx].status));
        ret = -1;
        goto out;
    }
...
int dlock_server::primary_cmd_handler()
{
    ...
    while (!m_stop) {
        ...
        if ((n < 0) || (n > MAX_NUM_CLIENT)) {
            DLOCK_LOG_ERR("urma_poll_jfc error (errno=%d %m)", errno);
            goto err;
        }
        if (n == 0) {
            continue;
...
static void *primary_control_launch(void *p_object)
{
    ...
    static_cast<void>(p_server->primary_control_loop());
    return nullptr;
}
...
{
    dlock_server *p_server = (dlock_server *)p_object;
    static_cast<void>(p_server->primary_cmd_handler());
    return nullptr;
}
...
jetty_mgr *dlock_server::init_client_primary(struct urma_init_body *jetty_info, bool /* reinit_flag */)
{
    ...
    p_jetty_mgr = create_jetty_mgr(m_p_urma_ctx, nullptr, CLIENT_PRIMARY, m_tp_mode);
