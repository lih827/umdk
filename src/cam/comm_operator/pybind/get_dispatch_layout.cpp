                                                            ...
                                                            int64_t numRanks)
{
    ...
    at::Tensor topk_idx_int64 = topk_idx.scalar_type() == at::kLong ? topk_idx : topk_idx.to(at::kLong);
    ...
    EXEC_NPU_CMD(aclnnDispatchLayout, topk_idx_int64, num_tokens, num_ranks, num_experts, num_topk, local_ranksize,
...
class ExtGetDispatchLayout : public torch::autograd::Function<ExtGetDispatchLayout> {
  public:
    static tensor_list forward(AutogradContext *ctx, const at::Tensor &topkIdx, int64_t numExperts, int64_t numRanks)
    {
