                                ...
                                const bool isEnableDiagnose)
{
    ...
    OPS_CHECK((xDesc->GetDataType() != ge::DT_BF16) && (xDesc->GetDataType() != ge::DT_FLOAT16),
              OPS_LOG_E(nodeName, "x dataType is invalid, dataType should be bf16 or float16, but is ."), return false);
    ...
    OPS_CHECK(expertIdDesc->GetDataType() != ge::DT_INT32,
              OPS_LOG_E(nodeName, "expertId dataType is invalid, dataType should be int32, but is ."), return false);
    ...
    if (quantMode != NO_SCALES) {
        OPS_CHECK(expandXDesc->GetDataType() != ge::DT_INT8,
                  OPS_LOG_E(nodeName, "expandX dataType is invalid, dataType should be int8, but is."), return false);
    } else {
    ...
    if (isEnableDiagnose) {
        ...
        OPS_CHECK(waitRecvCostStatsDesc == nullptr, OPS_LOG_E(nodeName, "dispatch waitRecvCostStatsDesc is null."),
                  return false);
        OPS_CHECK(waitRecvCostStatsDesc->GetDataType() != ge::DT_INT32,
                  OPS_LOG_E(nodeName,
                              ...
                              const bool isEnableDiagnose)
{
    ...
    auto expertIdDesc = context->GetInputDesc(EXPERT_IDS_INDEX);
    OPS_CHECK(expertIdDesc == nullptr, OPS_LOG_E(nodeName, "expertIdDesc is null."), return false);
    OPS_CHECK(static_cast<ge::Format>(ge::GetPrimaryFormat(expertIdDesc->GetStorageFormat())) == ge::FORMAT_FRACTAL_NZ,
              OPS_LOG_E(nodeName, "expertId format is invalid."), return false);

    auto expandXDesc = context->GetOutputDesc(OUTPUT_EXPAND_X_INDEX);
    OPS_CHECK(expandXDesc == nullptr, OPS_LOG_E(nodeName, "expandXDesc is null."), return false);
    OPS_CHECK(static_cast<ge::Format>(ge::GetPrimaryFormat(expandXDesc->GetStorageFormat())) == ge::FORMAT_FRACTAL_NZ,
    ...
    auto assistInfoDesc = context->GetOutputDesc(OUTPUT_ASSIST_INFO_INDEX);
    OPS_CHECK(assistInfoDesc == nullptr, OPS_LOG_E(nodeName, "assistInfoDesc is null."), return false);
    OPS_CHECK(static_cast<ge::Format>(ge::GetPrimaryFormat(assistInfoDesc->GetStorageFormat())) ==
                  ge::FORMAT_FRACTAL_NZ,
                                               ...
                                               std::string &groupTp)
{
    ...
    OPS_CHECK((moeExpertNum <= 0) || (moeExpertNum > MOE_EXPERT_MAX_NUM),
              ...
              return ge::GRAPH_FAILED);
    OPS_CHECK((*quantModePtr < static_cast<int64_t>(NO_SCALES)) ||
...
static ge::graphStatus MoeDispatchNormalA3TilingFuncImpl(gert::TilingContext *context)
{
    ...
    OPS_CHECK(TilingCheckMoeDispatchNormal(context, nodeName, quantMode, isEnableDiagnose) != ge::GRAPH_SUCCESS,
    ...
    OPS_CHECK(CheckTensorShape(context, nodeName, *tilingData, quantMode, static_cast<int64_t>(localMoeExpertNum)) !=
                  ge::GRAPH_SUCCESS,
    ...
    // Dispatch data region: token header aligned to 512, effective token length h_align_32b + scale(32b) +
    // triplet(3*4b)
    ...
    OPS_CHECK(SetWorkSpace(context, nodeName) != ge::GRAPH_SUCCESS, OPS_LOG_E(nodeName, "Tiling set workspace failed."),
              return ge::GRAPH_FAILED);
