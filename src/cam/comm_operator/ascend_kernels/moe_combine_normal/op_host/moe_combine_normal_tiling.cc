...
{
    OPS_CHECK(!CheckInputTensorDim(context, nodeName), OPS_LOG_E(nodeName, "param shape of input tensor is invalid"),
              return false);
...
static bool CheckTensorDataType(gert::TilingContext *context, const char *nodeName, const bool isEnableDiagnose)
{
    ...
    OPS_CHECK((topkWeightsDesc->GetDataType() != ge::DT_FLOAT),
              OPS_LOG_E(nodeName, "topkWeights dataType is invalid, dataType should be float, but is "), return false);
...
static bool CheckTensorFormat(gert::TilingContext *context, const char *nodeName, const bool isEnableDiagnose)
{
    auto recvXDesc = context->GetInputDesc(RECV_X_INDEX);
    OPS_CHECK(recvXDesc == nullptr, OPS_LOG_E(nodeName, "recvXDesc is null."), return false);
    OPS_CHECK(static_cast<ge::Format>(ge::GetPrimaryFormat(recvXDesc->GetStorageFormat())) == ge::FORMAT_FRACTAL_NZ,
                             ...
                             uint32_t localExpertNum)
{
    ...
    OPS_CHECK(xDim1 != recvXDim1,
              OPS_LOG_E(nodeName, "x's dim1 not equal to h, x's dim1 = %ld, h = %ld", xDim1, recvXDim1), return false);
                                                   ...
                                                   const bool isEnableDiagnose)
{
    ...
    OPS_CHECK(!CheckTensorFormat(context, nodeName, isEnableDiagnose), OPS_LOG_E(nodeName, "param Format is invalid"),
              return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
static ge::graphStatus MoeCombineNormalA3TilingFuncImpl(gert::TilingContext *context)
{
    ...
    OPS_CHECK(!CheckAttrs(context, *tilingData, nodeName, localMoeExpertNum), OPS_LOG_E(nodeName, "attr check failed."),
              return ge::GRAPH_FAILED);
